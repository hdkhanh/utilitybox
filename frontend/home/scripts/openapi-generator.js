...
const SPECS = [
    { path: "/auth-service/v3/api-docs", name: "auth-service" },
    { path: "/app-service/v3/api-docs", name: "app-service" },
];

const deleteExistSpec = async (spec) => {
    console.info(`Deleting...`);
    const generatedFolder = path.resolve(`${DEFAULT_OUTPUT_DIR}/${spec.name}`);
...
const run = async () => {
    try {
        ...
        await Promise.all(SPECS.map((spec) => {
            console.log(`${spec.name}: processing...`)
            deleteExistSpec(spec);
            downloadSpec(spec);
            generate(spec);
        }));
    } catch (e) {
